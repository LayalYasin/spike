You can use this document to be guided through the multiple steps to process new (HiSeq) sequencing data.
 ...
 4. **Execute spike**
    ...
    3. When ssh'ing to the HPC, you land on the head node. You should not do any serious computation on the machine, since it will very quickly negatively impact other users and the HPC will block you! To avoid this, request an interactive cluster session to get decend compute power `qsub -I -A ngsukdkohi -l select=1:ncpus=1:mem=10GB -l walltime=49:29:50`, which means one CPU for ~50 hours (`spike` should not take longer than that for your sampels) and 10 GB of RAM.
    
    4. `cd` into your spike install directory.
    
    5. execute snakemake, first as a dry run: `snakemake -p --cluster-config cluster.json --cluster "qsub -A {cluster.account} -q {cluster.queue} -l walltime={cluster.time} -l select={cluster.nodes}:ncpus={cluster.ppn}:mem={cluster.mem}"  -j 100 --latency-wait 900 --use-conda --cluster-status scripts/barnacle_status.py --max-status-checks-per-second 1 --keep-going -n`
    ...
    6. Trigger the actual multi parallel execution of the pipeline: `snakemake -p --cluster-config cluster.json --cluster "qsub -A {cluster.account} -q {cluster.queue} -l walltime={cluster.time} -l select={cluster.nodes}:ncpus={cluster.ppn}:mem={cluster.mem}"  -j 100 --latency-wait 900 --use-conda --cluster-status scripts/barnacle_status.py --max-status-checks-per-second 1 --keep-going`
